...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-JVMFBFQE.js");
    ...
    if (options.type === "erc721") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-5VQKNRWY.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-5VQKNRWY.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-364JCOOU.js");
...
//# sourceMappingURL=chunk-6UEENYEU.js.map
