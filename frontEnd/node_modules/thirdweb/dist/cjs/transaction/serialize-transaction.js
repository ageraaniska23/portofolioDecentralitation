...
const trim_js_1 = require("../utils/encoding/helpers/trim.js");
...
function serializeEIP1559Transaction(options) {
    (0, viem_1.assertTransactionEIP1559)(options.transaction);
    const { transaction: { chainId, gas, nonce, to, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data, r, s, v, yParity, }, } = options;
    const serializedAccessList = (0, viem_1.serializeAccessList)(accessList);
    ...
    return `0x02${(0, viem_1.toRlp)(serializedTransaction).slice(2)}`;
}
function serializeEIP2930Transaction(options) {
    (0, viem_1.assertTransactionEIP2930)(options.transaction);
    const { transaction: { chainId, gas, gasPrice, nonce, to, value, accessList, data, r, s, v, yParity, }, } = options;
    const serializedAccessList = (0, viem_1.serializeAccessList)(accessList);
    const serializedTransaction = [
        (0, hex_js_1.toHex)(chainId),
        nonce ? (0, hex_js_1.toHex)(nonce) : "0x",
        gasPrice ? (0, hex_js_1.toHex)(gasPrice) : "0x",
        gas ? (0, hex_js_1.toHex)(gas) : "0x",
        (to ?? "0x"),
        value ? (0, hex_js_1.toHex)(value) : "0x",
        data ?? "0x",
    ...
    return `0x01${(0, viem_1.toRlp)(serializedTransaction).slice(2)}`;
}
function serializeTransactionLegacy(options) {
    (0, viem_1.assertTransactionLegacy)(options.transaction);
    ...
    return (0, viem_1.toRlp)(serializedTransaction);
}
