...
class InAppNativeConnector {
    ...
    preAuthenticate(params) {
        ...
        switch (strategy) {
            case "email": {
                return (0, native_auth_js_1.sendVerificationEmail)({
                    email: params.email,
                    client: this.options.client,
                });
            }
            case "phone":
                return (0, native_auth_js_1.sendVerificationSms)({
    ...
    async validateEmailOTP(options) {
        try {
            const { storedToken } = await (0, native_auth_js_1.validateEmailOTP)({
                email: options.email,
                client: this.options.client,
                otp: options.otp,
